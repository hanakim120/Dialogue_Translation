...
def main(args):   
    ...
    extracted_persona_path = args.merged_persona_file
    with open(extracted_persona_path, 'r') as f:
        _extracted_persona_data = f.read()
        extracted_persona_data = eval(_extracted_persona_data)
    # load data
    ...
    if args.extract_persona_type == 'dialog-level':
        ...
        if args.dialog_type == "conversation" :
            ...
            for epi_idx, whole_dialog in enumerate(input_history_list): # 대화수
                ...
                A_persona_list = extracted_persona_data[epi_idx]['pred_A_summ']
                B_persona_list = extracted_persona_data[epi_idx]['pred_B_summ']
                model_input = prompt % (args.source_lang, args.target_lang, args.source_lang, whole_dialog, A_persona_list, B_persona_list, args.target_lang)   
                ...
                predict.append({
                    ...
                    'A_persona': A_persona_list,
                    'B_persona': B_persona_list,
                    'source' : whole_dialog,
